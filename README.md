...
2. Скопируйте файл `.env.example` в `.env` и при необходимости измените значения.
3. Запустите скрипт `scripts/start-docker.sh`:
   ```bash
   ./scripts/start-docker.sh
   ```
   Скрипт создаст директорию `secrets/` с тестовыми значениями (если она отсутствует) и поднимет контейнеры командой `docker-compose up -d --build`.

4. После старта приложение будет доступно на [http://localhost:8080](http://localhost:8080). Документация API: [http://localhost:8080/swagger-ui.html](http://localhost:8080/swagger-ui.html)

## 🧪 Запуск тестов

### Локально

1. Убедитесь, что Maven установлен. Запустить скрипт `scripts/setup.sh`, если Maven отсутствует.
2. Выполните
   ```bash
   mvn test
   ```
...
Репозиторий содержит GitHub Actions workflow `.github/workflows/tests.yml`. Тесты автоматически выполняются при каждом push или pull request в ветку `master`.
